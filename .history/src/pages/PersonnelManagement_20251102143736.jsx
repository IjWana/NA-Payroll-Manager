...
function normalizeRow(row) {
  ...
  const pick = (...names) => {
    for (const name of names) {
      ...
      if (normRow[key] !== undefined && normRow[key] !== null && String(normRow[key]).trim() !== '') return normRow[key];
    }
    return '';
  };
  return {
    employeeNo: pick('employeeNo', 'army no', 'army number', 'service number', 'employee no', 'armyno', 'armynumber', 'servicenumber', 'employeeno'),
    fullName: pick('fullName', 'name', 'staff name', 'personnel name', 'surname firstname', 'surnamefirstname'),
    rank: pick('rank'),
    corps: pick('corps'),
    fmnunit: pick('fmn/unit', 'fmn unit', 'unit', 'fmn', 'formation', 'fmnunit'),
    basicSalary: pick('basicSalary', 'salary', 'basic', 'basic salary'),
    allowance: pick('allowance', 'allowances'),
    bankName: pick('bankName', 'bank', 'bank name'),
    accountNumber: pick('accountNumber', 'account no', 'acct no', 'acct number', 'acctno', 'accountno', 'account number'),
    ...
    reader.onload = (e) => {
      try {
        ...
        for (let i = 0; i < json.length; i++) {
          ...
          const employeeNo = String(r.employeeNo || '').trim();
          const fullName = String(r.fullName || '').trim();
          if (!employeeNo || !fullName) {
            errors.push(i + 2); // row index (header + 1)
            continue;
          }
          ...
          additions.push({
            ...
            fmnunit: String(r.fmnunit || '').trim(),
            basicSalary: Number(r.basicSalary) || 0,
            allowance: Number(r.allowance) || 0,
            bankName: String(r.bankName || '').trim(),
            accountNumber: String(r.accountNumber || '').replace(/\D/g, '').slice(0, 12),
            status: String(r.status || 'Active').trim() || 'Active',
          });
        }
        setRows((prev) => {
          const existingKeys = new Set(prev.map((r) => String(r.employeeNo || '').trim().toLowerCase()));
          const toAdd = additions.filter((a) => !existingKeys.has(a.employeeNo.trim().toLowerCase()));
          const skipped = additions.length - toAdd.length;
          const imported = toAdd.length;
          const errored = errors.length;
          if (imported === 0) {
            alert(`No new records imported. Skipped ${skipped} duplicate(s). Errors: ${errored}.`);
            return prev;
          }
          alert(`Imported ${imported} record(s). Skipped ${skipped} duplicate(s). Errors: ${errored}.`);
          return [...toAdd, ...prev];
        });
      } catch (err) {
        console.error(err);
        alert('Failed to read the file. Ensure it is a valid Excel/CSV file.');
      }
    };
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <ArrowUpDown size={16} /> Import Excel
          </button>
          <input
            ref={fileInputRef}
            type="file"
